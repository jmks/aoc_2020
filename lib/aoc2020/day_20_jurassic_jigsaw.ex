...
"""
  ...
  def parse_tile(lines) do
    ...
    left = Enum.map(rest, &String.slice(&1, 0, 1)) |> Enum.join("")
    right = Enum.map(rest, &String.slice(&1, -1, 1)) |> Enum.join("")
  ...
  def edges(tiles) do
    Enum.reduce(tiles, %{}, fn %{id: tile, sides: sides}, map ->
      possible_sides = Enum.flat_map(sides, fn side -> [side, String.reverse(side)] end)

      Enum.reduce(possible_sides, map, fn s, m ->
        Map.update(m, s, [tile], fn tiles -> [tile | tiles] end)
      end)
    end)
  end
